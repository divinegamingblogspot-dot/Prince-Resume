...
(()=> {
  ...
  const commands=[['01','Home','Go to the hero','index.html#home'],['02','About','Open profile','index.html#about'],['03','Experience','View career history','index.html#experience'],['04','Systems','View build work','index.html#systems'],['05','Skills','Explore capability stack','index.html#skills'],['06','Training','Open training','index.html#training'],['07','Contact','Get in touch','index.html#contact'],['08','Recruiter mode','Open hiring summary','__recruiter'],['09','Resume','Open printable resume','resume.html'],['10','Nova','Open AI assistant','__nova'],['11','GitHub','Open GitHub profile','https://github.com/divinegamingblogspot-dot']];
  ...
  const run=a=>{close();if(a==='__recruiter'){document.getElementById('recruiterBtn')?.click();return}if(a==='__nova'){document.querySelector('#pageBot')?.scrollIntoView({behavior:reduce?'auto':'smooth'});document.querySelector('#botInput')?.focus();return}if(/^https?:/.test(a)){window.open(a,'_blank','noopener');return}const hash=a.split('#')[1],el=hash&&document.getElementById(hash);if(el)el.scrollIntoView({behavior:reduce?'auto':'smooth'});else location.href=a};
