...
/* ===== NOVA COGNITIVE CORE v2 ===== */
(()=>{
 const form=document.getElementById('botForm'),input=document.getElementById('botInput'),messages=document.getElementById('botMessages');
 if(!form||!input||!messages||window.__novaCognitiveV2)return; window.__novaCognitiveV2=true;
 const STORE='prince_nova_cognitive_v2';
 const P={name:'Prince Dixit',age:22,location:'Delhi, India',focus:'E-commerce Operations × Digital × Automation',contact:'demonicspirit888@gmail.com · +91 88878 31825',education:'BA (Hons.) Economics at Banaras Hindu University; CUET Reasoning 99.43 percentile; Class 12 Commerce; Digital Marketing training.',experience:'Multybyte Marketing India — E-commerce Operations (May 2026–Present); Crafts Banaras — E-commerce Manager; Paraxion Management & Consultant Pvt. Ltd. — Telesales Executive; Unique Threads Sarees — Orders & Inventory Manager.',skills:'E-commerce Operations, Google Sheets, Apps Script, JavaScript, HTML/CSS, APIs, SEO, Digital Marketing, AI Workflows and business automation.',systems:'Multybyte Automation, Operational Spreadsheet Systems, AI / Software Experiments, ME N U, EyeNav → Doc and the Prince-Resume portfolio system.',multybyte:'Prince works with product/SKU data, purchasing coordination, vendor workflows, inventory, warehouse coordination, packing/dispatch and Google Sheets + Apps Script automation with retries, caching, locks and backup/recovery.',doc:'EyeNav evolved toward Doc, an Android voice-assistant concept using “hey doc” / “doc” commands with background and locked-screen command goals.',menu:'ME N U is a private relationship-focused Google Sheets/AI project with TODAY, MEMORIES, FUTURE and SECRET areas, tasks, coins and assistant interactions.',portfolio:'Prince-Resume is a multi-page portfolio/resume system with dedicated experience, systems, skills, recruiter, resume, Nova and contact pages.'};
 const clean=v=>String(v||'').toLowerCase().normalize('NFKD').replace(/[\u0300-\u036f]/g,'').replace(/[’‘'“”"!?.,:;()\[\]{}]/g,' ').replace(/\s+/g,' ').trim();
 const hi=v=>/\b(kya|kaun|kon|kahan|kaha|kyu|kyun|kaise|kitna|kitne|uska|iske|batao|bata|hai|hain|tha|thi|mein|me|ke|ki|ka|se|ko|par|aur|wala|wali|mujhe|tum|aap|prince ke|prince ka|prince ki|nova kya|nova kaun)\b/i.test(v);
 ...
 function self(s,h){
  if(has(s,['are you conscious','are u conscious','do you have consciousness','do you feel','do you have feelings','are you alive','are you sentient','do you have a mind','are you real']))return h?'Nahi, literal sense me nahi. Main AI software interface hoon—mere paas biological consciousness, feelings ya private inner experience nahi hai. Lekin main apni role, knowledge aur conversation context ko model karke natural, self-aware style me respond kar sakti hoon.':'Not literally. I’m an AI software interface—I don’t have biological consciousness, feelings, or a private inner experience. I can model my role, knowledge and conversation context and respond in a natural, self-aware style.';
  if(has(s,['who are you','what are you','tell me about yourself','about yourself','introduce yourself','tum kaun','aap kaun','nova kaun']))return h?'Main Nova hoon — Prince ke portfolio ki AI assistant. Main Prince, uske projects aur is website ko explain karti hoon, follow-up context rakhti hoon, aur available knowledge se questions reason karti hoon.':'I’m Nova — the AI assistant for Prince’s portfolio. I explain Prince, his projects and this website, keep short conversation context, and reason from the knowledge available to me.';
  if(has(s,['why were you created','why do you exist','your purpose','what is your purpose','tumhe kyu banaya','tumhara purpose']))return h?'Mera purpose visitors ko Prince aur uske work ko conversation ke through samjhana hai—sirf fixed buttons ke through nahi.':'My purpose is to help visitors understand Prince and his work conversationally rather than forcing them through fixed menus.';
  if(has(s,['what can you do','what can you answer','your capabilities','tum kya kar sakti']))return h?'Main Prince-related questions, portfolio context, projects, skills, career, education, Nova ke baare me questions, English/Hindi/Hinglish phrasing aur follow-ups handle karti hoon. Jo fact mere verified knowledge me nahi hai, usse invent nahi karungi.':'I can handle Prince-related questions, portfolio context, projects, skills, career, education, questions about Nova, English/Hindi/Hinglish phrasing and follow-ups. If a fact is not in my verified knowledge, I won’t invent it.';
  if(has(s,['do you know prince personally','are you prince','are you his clone','are you a human']))return h?'Nahi. Main Prince nahi hoon aur human bhi nahi. Main uske portfolio me chalne wali AI assistant hoon.':'No. I’m not Prince and I’m not human. I’m the AI assistant running inside his portfolio.'; return null;
 }
 function topic(s,h){
  if(has(s,['who is prince','tell me about prince','about prince','prince kaun','prince kon','prince ke bare','prince ke baare','prince k bare']))return h?'Prince Dixit Delhi, India based professional hain, jinka focus E-commerce Operations × Digital × Automation hai. Unka kaam real business operations ko spreadsheets, Apps Script, JavaScript, SEO, digital work aur practical software/AI systems se connect karta hai.':'Prince Dixit is a Delhi-based professional focused on E-commerce Operations × Digital × Automation. His work connects real business operations with spreadsheets, Apps Script, JavaScript, SEO, digital work and practical software/AI systems.';
  if(has(s,['age','how old','umar','kitne saal']))return h?'Prince ki age 22 hai.':'Prince is 22.';
  if(has(s,['where is prince','where does prince live','location of prince','prince kaha','prince kahan']))return h?'Prince Delhi, India me based hain.':'Prince is based in Delhi, India.';
  if(has(s,['contact prince','reach prince','prince email','prince phone','prince ka number','prince ka email']))return h?'Prince se demonicspirit888@gmail.com ya +91 88878 31825 par contact kiya ja sakta hai.':'Prince can be contacted at demonicspirit888@gmail.com or +91 88878 31825.';
  if(has(s,['education','degree','college','university','bhu','cuet','padhai','qualification']))return h?'Prince ne Banaras Hindu University se BA (Hons.) Economics kiya hai, CUET Reasoning me 99.43 percentile score kiya, Class 12 Commerce complete kiya aur Digital Marketing training li hai.':P.education;
  if(has(s,['experience','career','work history','worked','job','jobs','company','companies','naukri','kaam']))return h?'Unka documented experience Multybyte Marketing India, Crafts Banaras, Paraxion Management & Consultant Pvt. Ltd. aur Unique Threads Sarees me raha hai. Current role Multybyte me E-commerce Operations hai.':P.experience;
  if(has(s,['skills','skillset','technology','technologies','tech stack','stack','capabilities','expertise','kya aata']))return h?'Core skills: E-commerce Operations, Google Sheets, Apps Script, JavaScript, HTML/CSS, APIs, SEO, Digital Marketing, AI Workflows aur business automation.':P.skills;
  if(has(s,['multybyte','sku','vendor workflow','product workflow','purchasing','warehouse','inventory','packing','dispatch']))return P.multybyte;
  if(has(s,['eyenav','eye nav','doc android','hey doc','voice assistant','android assistant']))return P.doc;
  if(has(s,['me n u','me nu','menu project','relationship sheet']))return P.menu;
  if(has(s,['portfolio','website','resume system','github portfolio']))return P.portfolio;
  if(has(s,['projects','systems','what did he build','what has he built','builds','kya banaya']))return h?'Prince ke main systems Multybyte Automation, Operational Spreadsheet Systems, AI/Software Experiments, ME N U, EyeNav → Doc aur ye portfolio/resume system hain.':P.systems;
  if(has(s,['nova look','nova model','nova appearance','nova physical','nova dikhti','nova ka look']))return h?'Nova ka current physical UI intentionally basic hai. Intelligence layer alag hai; visual model bina explicit change ke basic rahega.':'Nova’s current physical UI is intentionally basic. Her intelligence layer is separate from her appearance, and the visual model remains basic unless explicitly changed.';
  if(has(s,['what does prince think','what is prince like','prince personality','prince kaisa']))return h?'Portfolio me documented work-style hai: observe → break down → automate → validate → improve. Personal traits ke baare me main assumptions nahi banaungi.':'The portfolio documents a work pattern of observe → break down → automate → validate → improve. I won’t invent undocumented personality claims.';
  if(has(s,['salary','income','girlfriend','relationship','address','home address','private','password']))return h?'Ye detail Prince ke public portfolio knowledge me documented nahi hai, isliye main guess nahi karungi.':'That detail is not part of Prince’s public portfolio knowledge, so I won’t guess or expose private information.'; return null;
 }
 function general(s,h){if(/\b(ai|artificial intelligence)\b/.test(s))return h?'AI aise software systems hain jo language, patterns, decisions ya generation jaise tasks kar sakte hain.':'AI refers to software systems that can perform tasks such as language understanding, pattern recognition, decision support or generation.';if(has(s,['what is seo','seo kya']))return h?'SEO ka matlab Search Engine Optimization hai—website ko search engines ke liye understandable aur useful banana.':'SEO means Search Engine Optimization: improving a website so search engines can understand and surface its useful pages.';if(has(s,['what is javascript','javascript kya']))return h?'JavaScript website aur software me logic, interaction aur dynamic behavior handle karta hai.':'JavaScript handles logic, interaction and dynamic behavior in websites and software.';if(has(s,['what is api','api kya']))return h?'API ek defined interface hai jisse software systems data exchange ya actions trigger karte hain.':'An API is a defined interface that lets software systems exchange data or trigger actions.';return null}
 function fallback(s,h){const more=has(s,['tell me more','more about that','go deeper','elaborate','explain more','aur batao','aur btao','iske bare','uske bare','thoda aur','detail me','details']);if(more&&state.answer)return h?'Bilkul. Pichhle topic ke context me: '+state.answer:'Sure. Going deeper from the previous topic: '+state.answer;return h?'Question samajh gayi, lekin ye specific fact mere verified Prince/portfolio knowledge me nahi hai. Hindi, Hinglish ya English me rephrase karo—main available facts se best answer dungi.':'I understand the question, but that specific fact is not in my verified Prince/portfolio knowledge. Rephrase it in English, Hindi or Hinglish and I’ll work from the facts I actually have.'}
 function answer(q){const s=clean(q),h=hi(q);let a=self(s,h)||topic(s,h)||general(s,h)||fallback(s,h);state.topic=s;state.answer=a;state.lang=h?'hi':'en';save();return a}
 ...
 function submit(e){const q=input.value.trim();if(!q)return;e.preventDefault();e.stopImmediatePropagation();add(q,'user');input.value='';setTimeout(()=>{const a=answer(q);add(a,'bot');try{const old=JSON.parse(sessionStorage.getItem('prince_nova_session_v3')||'[]');old.push({role:'user',text:q},{role:'bot',text:a});sessionStorage.setItem('prince_nova_session_v3',JSON.stringify(old.slice(-14)))}catch{}},window.matchMedia('(prefers-reduced-motion:reduce)').matches?20:100)}
 form.addEventListener('submit',submit,true);window.NovaCognitive={answer,profile:P};
})();
